...
A command-line pipeline that converts books into audiobooks using LLM-based text
cleaning and neural text-to-speech.
...
2. **clean** — Clean and normalize the raw text using an LLM, splitting it into
   chapters
...
Each stage reads from a directory and writes to another, so you can run them
independently or skip stages that don't apply to your input.
...
The `TOKENIZER` path points to the ChatterboxTTS tokenizer, which is downloaded
automatically by HuggingFace when you first run `generate.py`. Check
`~/.cache/huggingface/hub/` for the path after first run.
...
Generates WAV audio for each chapter using
[ChatterboxTTS](https://github.com/resemble-ai/chatterbox). Text is split into
~200-token chunks internally; chunks are cached by content hash so interrupted
...
Chapters already present in the output directory are skipped. The script detects
Apple Silicon (MPS) automatically and falls back to CPU.

---

### `convert.py` — WAV to Opus

Converts WAV files to Opus using ffmpeg. Files already present in the output
